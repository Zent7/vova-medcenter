...
def spoil_and_replace_form(
    ...
    *,
...
) -> tuple[BlankForm, BlankForm | None]:
    """Списывает текущий бланк в брак и подбирает следующий номер той же серии."""
    ...
    if form is None:
        raise BlankServiceError("Бланк не найден")
    if form.status not in {BLANK_STATUS_FREE, BLANK_STATUS_ISSUED}:
        raise BlankServiceError("Заменить можно только свободный или выданный бланк")

    spoiled_at = datetime.utcnow()
    spoiled_reason = (reason or "").strip() or "Бракованный бланк"
    ...
    for document in generated_documents:
        if document.cancelled_at is None:
            document.cancelled_at = spoiled_at
            document.cancelled_by_user_id = user_id
            document.cancelled_reason = spoiled_reason

    form.status = BLANK_STATUS_SPOILED
    form.spoiled_at = spoiled_at
    form.spoiled_by_user_id = user_id
    form.spoiled_reason = spoiled_reason
    db.flush()
    ...
    write_audit_log(
        ...
        payload_json={
            ...
            "generated_document_ids": [document.id for document in generated_documents],
            "reason": spoiled_reason,
        },
    )

    next_form = get_next_free_form(
        db,
        blank_type=form.blank_type,
        center_id=form.center_id,
        series=form.series,
    )
    if next_form is None and auto_create:
        next_form = create_auto_number_form(
            db,
            blank_type=form.blank_type,
            center_id=form.center_id,
            series=form.series or "",
            user_id=user_id,
        )
    return form, next_form
