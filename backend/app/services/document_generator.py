...
def _prof_extract_doctor_name(client: Client | None, role_id: str, exam_data: dict[str, object]) -> str:
    exam_doctor = str(exam_data.get("doctor") or "").strip()
    generic_exam_names = {
        str(role_id or "").strip().casefold(),
        next(
            (
                specialty.casefold()
                for candidate_role_id, specialty, _ in PROF_EXTRACT_DOCTOR_ROWS
    ...
    if exam_doctor and exam_doctor.casefold() not in generic_exam_names:
        return exam_doctor
