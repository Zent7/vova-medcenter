...
// Напечатанный в этой вкладке документ лежит ещё и в data.documents — копией
// ответа печати, из которой журнал берёт номер справки. Бланк потом могут
// списать («Нет» в окне результата печати) или освободить: сервер отменяет
// документ, а копия об этом не знала, и журнал показывал номер списанного
// бланка как выданный. Копия повторяет серверную версию того же документа.
function syncLocalDocumentsWithServer(serverDocuments) {
  const serverDocumentsById = new Map(
    serverDocuments
      .filter((documentItem) => documentItem.backendId)
      .map((documentItem) => [String(documentItem.backendId), documentItem]),
  );
  (data.documents || []).forEach((documentItem) => {
    const serverDocument = documentItem.backendId ? serverDocumentsById.get(String(documentItem.backendId)) : null;
    if (!serverDocument) return;
    documentItem.blankFormId = serverDocument.blankFormId;
    documentItem.blankNumber = serverDocument.blankNumber;
    documentItem.cancelledAt = serverDocument.cancelledAt;
    documentItem.cancelledReason = serverDocument.cancelledReason;
  });
}

// Освобождённый номер сервер снимает со всех документов бланка и отменяет их.
// Раздел «Бланки» после этого перечитывает только бланки, поэтому загруженные
// документы повторяют освобождение сами, иначе номер остаётся в журнале.
function releaseBlankInLoadedDocuments(blankFormId) {
  if (!blankFormId) return;
  const releasedAt = new Date().toISOString();
  [...(data.generatedDocuments || []), ...(data.documents || [])].forEach((documentItem) => {
    if (String(documentItem.blankFormId || "") !== String(blankFormId)) return;
    documentItem.blankFormId = null;
    documentItem.blankNumber = "";
    documentItem.cancelledAt = documentItem.cancelledAt || releasedAt;
  });
}
...
async function loadWorkflowData(options = {}) {
  ...
  try {
    ...
    syncLocalDocumentsWithServer(data.generatedDocuments);
        ...
        <div class="import-help">
          <div>
            ...
            <ol>
              ...
              <li>Завод заполняет жёлтые колонки: фамилию, имя и дату рождения.</li>
              <li>Выбрать заполненный файл ниже.</li>
              <li>Сначала посмотреть предпросмотр, потом загрузить в базу.</li>
              <li>Проверить услуги: из колонки «Тип документа» подставятся только однозначные.</li>
            </ol>
          </div>
          <div>
            ...
            <ul>
              ...
              <li>собирать адрес регистрации из колонок «область / город / улица / дом / корпус / квартира»;</li>
              <li>переносить должность, вредные производственные факторы и дату ФЛГ;</li>
...
async function refreshDocumentWorkflowState(clientId, encounterId) {
  ...
  // Номер справки строки журнала сервер берёт из неотменённых документов
  // обращения. После печати, списания или освобождения бланка перечитываем его,
  // иначе строка держит номер, полученный до списания.
  void loadDashboardDoctorStatuses(getVisibleDashboardClients());
}
...
window.releaseBlankInLoadedDocuments = releaseBlankInLoadedDocuments;
